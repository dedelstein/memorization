...
def train_classifier(args):
    ...
    trainer = pl.Trainer(
        ...
        else 1,  # Use 1 device regardless of hardware type
        precision="16-mixed"
        if torch.cuda.is_available()
        else 32,  # Use mixed precision only on GPU
        logger=logger,
    ...
    trainer.save_checkpoint(
        os.path.join("checkpoints", "classifier", "classifier_final.ckpt")
    )
...
def main():
    ...
    parser.add_argument(
        "--channels", type=int, default=64, help="Base channel multiplier"
    )
    parser.add_argument(
        "--time_dim", type=int, default=256, help="Time embedding dimension"
    )
