...
class ClassifierGuidedDiffusion(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        """
        images, _ = batch  # We don't use labels during validation for classifier guidance
        ...
        with torch.no_grad():
            for i, t in enumerate(ddim_scheduler.timesteps):
                ...
                if classifier_scale > 0:
                    with torch.enable_grad():
                        gradient = self.classifier_gradient(latents, timestep, labels, classifier_scale)
                else:
                    gradient = 0
                ...
                latents = alpha_prod_t_prev ** 0.5 * pred_original_sample + \
