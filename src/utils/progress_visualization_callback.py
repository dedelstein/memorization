...
class ProgressVisualizationCallback(Callback):
    ...
    def on_epoch_end(self, trainer, pl_module):
        ...
        try:
            ...
            for i, cond_idx in enumerate(self.condition_indices):
                ...
                labels = torch.zeros((self.num_samples, len(CHEXPERT_CLASSES)), device=pl_module.device)
                labels[:, cond_idx] = 1.0
                ...
                if self.fixed_noise:
                    ...
                    samples = pl_module.generate_samples(
                        ...
                        initial_noise=self.fixed_noises[cond_name].clone()  # Use copy to not alter the original
                    )
                else:
    ...
    def _create_timeline(self, trainer, pl_module):
        ...
        for i, cond_name in enumerate(self.conditions):
            fig, axes = plt.subplots(1, len(available_epochs), figsize=(len(available_epochs) * 3, 3))
