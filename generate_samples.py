...
def save_images(images, output_dir, prefix="sample", title=None):
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, f"{prefix}_grid.png"),
        bbox_inches='tight'
    )
    plt.close()
...
def main():
    ...
    parser.add_argument('--classifier_path', type=str, default=None, help='Path to the classifier (for CG)')
    
    # Generation parameters
    parser.add_argument('--output_dir', type=str, default='generated_samples', help='Output directory')
    parser.add_argument('--num_samples', type=int, default=4, help='Number of samples to generate per condition')
    parser.add_argument('--guidance_scale', type=float, default=3.0, help='Guidance scale (higher = stronger conditioning)')
    parser.add_argument('--inference_steps', type=int, default=50, help='Number of inference steps')
    
    # Condition parameters
    parser.add_argument('--conditions', type=int, nargs='+', default=None, help='Label indices to condition on')
    parser.add_argument('--list_classes', action='store_true', help='List available class indices for conditioning')
