...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(train_dataloader, desc=f"Epoch {epoch + 1}/{num_epochs}") as pbar:
            for batch_idx, batch in enumerate(pbar):
                with accelerator.accumulate(model):
                    ...
                    t = torch.randint(
                        0, diffusion.steps, (x.shape[0],), device=accelerator.device
                    )
                    ...
                    if (
                        batch_idx + 1
                    ) % gradient_accumulation_steps == 0 or batch_idx == len(
                        train_dataloader
                    ) - 1:
                        optimizer.step()
                        optimizer.zero_grad()
                    ...
                    if run is not None:
                        run["train/batch_loss"].log(
                            loss.item(), step=epoch * len(train_dataloader) + batch_idx
                        )
        ...
        print(
            f"Epoch {epoch + 1}/{num_epochs}, Avg Train Loss: {avg_train_loss:.6f}, Val Loss: {val_loss:.6f}"
        )
...
def main():
    ...
    optimizer = torch.optim.AdamW(params_to_optimize, lr=1e-4, weight_decay=1e-5)
